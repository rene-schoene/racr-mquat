...
;   (set!no-frequency) ; only use property load for system creation
      ...
      (let* ([ast (create-system 2 0 1 1 2 (list #f no-freq-sw-clauses no-freq-hw-clauses #f))])
        ...
        (case id
          ...
          [(4) ; Description: mode-1-1-1 does not meet request constraint (max-eq)
           ...
           (change-sw-prov ast 'prop-1 7 'm-1-1-1) ; max-req not met
           (change-sw-prov ast 'prop-1 3 'm-1-1-2)]
          [(5) ; Description: mode-1-1-1 does not meet request constraint (min-eq)
           ; Expected outcome: second mode (mode-1-1-2) is deployed on either res-1 or res-2
           (change-sw-req ast 'load comp-max-eq 0.8)
           (change-req-constraint ast 'prop-1 comp-min-eq 4)
           (change-sw-prov ast 'prop-1 3 'm-1-1-1) ; min-req not met
           (change-sw-prov ast 'prop-1 7 'm-1-1-2)]
          ...
          [(12) ; Description: Reqs only met on res-2, mode-1-1-2 does not meet request constraint (req:min-eq and res:max-eq)
           ...
           (change-hw-prov ast 'load 0.9 'r-1)
           (change-req-constraint ast 'prop-1 comp-min-eq 4)
           (change-sw-prov ast 'prop-1 7 'm-1-1-1)
           (change-sw-prov ast 'prop-1 3 'm-1-1-2) ; min-req not met
           (change-sw-req ast 'load comp-max-eq 0.8)] ; max-req not met on res-1
          ...
          [(15) ; Description: Dynamic provision function, mp-value + 5. mp-value = 10. Required >= 14.
           ...
           (change-sw-prov ast 'prop-1 (lambda (lomp target) (+ 5 (=value-of lomp mp-name))) 'm-1-1-1)
           (change-sw-prov ast 'prop-1 20 'm-1-1-2)]
      ...
      (let ([ast (create-system 2 0 1 2 1 (list #f no-freq-sw-clauses no-freq-hw-clauses #f))])
        ...
        (case id
          ...
          [(33) ; mode-1-1-1 does not meet request constraint (max-eq)
           ...
           (change-req-constraint ast 'prop-1 comp-max-eq 4)
           (change-sw-prov ast 'prop-1 7 'm-1-1-1)
           (change-sw-prov ast 'prop-1 3 'm-1-2-1)]
          [(34) ; mode-1-1-1 does not meet request constraint (min-eq)
           ; Expected outcome: second mode (mode-1-2-1) is deployed on res-1 or res-2
           (change-req-constraint ast 'prop-1 comp-min-eq 4)
           (change-sw-prov ast 'prop-1 3 'm-1-1-1)
           (change-sw-prov ast 'prop-1 7 'm-1-2-1)]
   ...
   (let ([ast (create-system 3 0 1 2 2 (list #f no-freq-sw-clauses no-freq-hw-clauses #f))])
     ...
     (case id
       ...
       [(118) ; Description: Reqs only met on res-2 and res-3
        ...
        (change-sw-prov ast 'prop-1 3 'm-1-1-1) ; min-req not met
        (change-sw-prov ast 'prop-1 2 'm-1-2-1)] ; min-req not met
        ...
        (change-sw-prov ast 'prop-1 8 'm-1-1-1) ; max-req not met
        (change-sw-prov ast 'prop-1 7 'm-1-2-1)] ; max-req not met
        ...
        (change-sw-prov ast 'prop-1 1)
        (change-sw-prov ast 'prop-1 8 'm-1-1-1) ; max-req not met
        (change-sw-prov ast 'prop-1 7 'm-1-1-2)] ; max-req not met
        ...
        (change-sw-prov ast 'prop-1 1 'm-1-1-1) ; max-req not met
        (change-sw-prov ast 'prop-1 2 'm-1-1-2)] ; max-req not met
        ...
        (change-req-constraint ast 'prop-1 comp-min-eq 4)
        (change-sw-prov ast 'prop-1 4)
        (change-sw-prov ast 'prop-1 1 'm-1-1-1) ; min-req not met
        (change-sw-prov ast 'prop-1 2 'm-1-1-2)] ; min-req not met
       [(127 128 129)
        ; Description: Dynamic provision function, mp-value + 5|10|15|20.
        (change-req-constraint ast 'prop-1 comp-min-eq 30)
        (change-sw-prov ast 'prop-1 (lambda (lomp target) (+  5 (=value-of lomp mp-name))) 'm-1-1-1)
        (change-sw-prov ast 'prop-1 (lambda (lomp target) (+ 10 (=value-of lomp mp-name))) 'm-1-1-2)
        (change-sw-prov ast 'prop-1 (lambda (lomp target) (+ 15 (=value-of lomp mp-name))) 'm-1-2-1)
        (change-sw-prov ast 'prop-1 (lambda (lomp target) (+ 20 (=value-of lomp mp-name))) 'm-1-2-2)
        (case id
 ...
 (define (two-comps id)
   ...
   (let ([ast (create-system 3 0 2 2 2 (list (lambda (impl) #t) no-freq-sw-clauses no-freq-hw-clauses #f))])
     ...
     (change-sw-req ast 'prop-2 comp-max-eq 20 'm-1-1-1 'm-1-1-2 'm-1-2-1 'm-1-2-2)
     (change-sw-prov ast 'prop-2 10 'm-2-1-1 'm-2-1-2 'm-2-2-1 'm-2-2-2)
     (case id
       ...
       [(201) ; Description: only impl-2-2 meets requirement of comp-1
        ...
        (change-sw-req ast 'load comp-max-eq 0.8)
        (remove-req-constraints ast)
        (change-sw-prov ast 'prop-2 40 'm-2-1-1 'm-2-1-2)] ; max-req not met
       [(202) ; Description: mode-2-1-1 not meet hw-reqs (max-eq), mode-1-1-1 not meet request constraint (max-eq)
        ...
        (change-req-constraint ast 'prop-1 comp-max-eq 4)
        (change-sw-prov ast 'prop-1 7 'm-1-1-1) ; max-req not met
        (change-sw-prov ast 'prop-1 4 'm-1-1-2 'm-1-2-1 'm-1-2-2)
        (change-sw-prov ast 'prop-2 40 'm-2-2-1 'm-2-2-2)] ; max-req not met
       ...
       [(205 206 207) ; Description: Dynamic provision function, mp-value +/- 5|10|15|20 for comp-1/comp-2.
        ...
        (change-sw-prov ast 'prop-1 (lambda (lomp target) (+ 15 (=value-of lomp mp-name))) 'm-1-2-1)
        (change-sw-prov ast 'prop-1 (lambda (lomp target) (+ 20 (=value-of lomp mp-name))) 'm-1-2-2)
        (change-sw-prov ast 'prop-2 (lambda (lomp target) (- 60 (=value-of lomp mp-name))) 'm-2-1-1)
        (change-sw-prov ast 'prop-2 (lambda (lomp target) (- 45 (=value-of lomp mp-name))) 'm-2-1-2)
        (change-sw-prov ast 'prop-2 (lambda (lomp target) (max 1 (- 31 (=value-of lomp mp-name)))) 'm-2-2-1)
        (change-sw-prov ast 'prop-2 (lambda (lomp target) (max 1 (- 15 (=value-of lomp mp-name)))) 'm-2-2-2)
 ...
 (define (two-comps-reqc id)
   ...
   (let ([ast (create-system 3 0 2 2 2 (list (lambda (impl) (eq? impl 'i-1-1))
     ...
     (change-sw-prov ast pn-energy 40 'm-2-2-1)
     (change-sw-prov ast pn-energy 45 'm-2-2-2)
     (change-sw-req ast 'prop-2 comp-max-eq 20 'm-1-1-1 'm-1-1-2)
     (change-sw-prov ast 'prop-2 10 'm-2-1-1 'm-2-1-2 'm-2-2-1 'm-2-2-2)
     (case id
 ...
 (define (two-resource-types id)
   ...
   (let* ([hw-types (lambda (res-name) (if (or (eq? res-name 'r-1) (eq? res-name 'r-3)) 1 #f))]
     ...
     (case id
       ...
       [(401) ; Impl-1-1 does not meet request constraint
        ...
        (change-sw-prov ast 'prop-1 3 'm-1-1-1 'm-1-1-2)
        (change-sw-prov ast 'prop-1 7 'm-1-2-1 'm-1-2-2)
       ...
       [(403) ; Dynamic provision value for requested property only for first three modes
        ...
        (change-sw-prov ast 'prop-1 (lambda (lomp target) 7) 'm-1-2-2)
        (change-req-constraint ast 'prop-1 comp-min-eq 5)]
       [(404) ; Low load of 0.2
   ...
   (case id
     ...
     [(904) ; Requirement on second component not met by any of its modes (max-eq)
      (let [(ast (create-system 3 0 2 2 2 (list (lambda _ #t) #f #f #f)))]
        (change-sw-req ast 'load comp-max-eq 0.8)
        (change-sw-prov ast 'prop-2 7 'm-2-1-1 'm-2-1-2 'm-2-2-1 'm-2-2-2)
        (change-sw-req ast 'prop-2 comp-max-eq 4 'm-1-1-1 'm-1-1-2 'm-1-2-1 'm-1-2-2)
        (save-ilp tmp-lp ast))]
     [(905) ; Requirement on second component not met by any of its modes (min-eq)
      (let [(ast (create-system 3 0 2 2 2 (list (lambda _ #t) #f #f #f)))]
        (change-hw-prov ast 'load 0.8)
        (change-sw-req ast 'load comp-max-eq 0.3)
        (change-sw-prov ast 'prop-2 2 'm-2-1-1 'm-2-1-2 'm-2-2-1 'm-2-2-2)
        (change-sw-req ast 'prop-2 comp-min-eq 15 'm-1-1-1 'm-1-1-2 'm-1-2-1 'm-1-2-2)
        (save-ilp tmp-lp ast))]
     [(906) ; Requirement on third component not met by any of its modes (max-eq)
      (let [(ast (create-system 3 0 3 2 2 (list (lambda _ #t) #f #f #f)))]
        (change-sw-req ast 'load comp-max-eq 0.8)
        (change-sw-prov ast 'prop-3 7 'm-3-1-1 'm-3-1-2 'm-3-2-1 'm-3-2-2)
        (change-sw-req ast 'prop-3 comp-max-eq 4 'm-2-1-1 'm-2-1-2 'm-2-2-1 'm-2-2-2)
        (save-ilp tmp-lp ast))]
     [(907) ; Requirement on third component not met by any of its modes (min-eq)
      (let [(ast (create-system 3 0 3 2 2 (list (lambda _ #t) #f #f #f)))]
        (change-hw-prov ast 'load 0.8)
        (change-sw-req ast 'load comp-max-eq 0.3)
        (change-sw-prov ast 'prop-3 2 'm-3-1-1 'm-3-1-2 'm-3-2-1 'm-3-2-2)
        (change-sw-req ast 'prop-3 comp-min-eq 15 'm-2-1-1 'm-2-1-2 'm-2-2-1 'm-2-2-2)
        (save-ilp tmp-lp ast))]
