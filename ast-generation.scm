...
(library
 ...
 (define (create-sw load freq num-comp impl-per-comp mode-per-impl as-reqs)
   ...
   (let* ([node-name (lambda (outer-id n) (string->symbol (string-append (symbol->string outer-id) "-" (number->string n))))]
          [make-sw-prop (lambda (n) (list (:Property mquat-spec (node-name 'prop n) 'u 'runtime 'increasing 'max)
          ...
          [make-mode (lambda (comp impl mode) 
                       (debug "make-mode" comp impl mode)
                       (let* ([ps (prop comp)]
          ...
          [make-impl (lambda (comp comp-name impl)
                       (debug "make-impl" comp comp-name impl)
                       (let ([name (node-name comp-name impl)])
                         (:Impl
                          mquat-spec name
